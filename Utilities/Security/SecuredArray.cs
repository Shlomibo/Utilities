...
namespace Utilities.Security
{
		...
		where T : struct
	{
		...
		{
			return this.pointer + (index * this.@sizeof);
		}
		...
		private void ThrowIfDisposed()
		{
			if (this.IsDisposed)
			{
				throw new ObjectDisposedException("The scured array is disposed", (Exception)null);
			}
		}
		...
		private bool CheckType(Type type)
		{
			...
			else if (type.IsValueType)
			{
				...
				if (!isOk)
				{
					...
					{
						if (!field.IsLiteral && !CheckType(field.FieldType))
						{
							isOk = false;
							break;
						}
		...
		{
			ThrowIfDisposed();
			return this.IndexOf(item) != NOT_FOUND;
		}
